...
class AnomalyDetector:
    ...
    def _check_isolation_forest_anomalies(self, message_name: str, signals: Dict[str, float]):
            ...
            if prediction[0] == -1:  # Anomali tespit edildi
                ...
                if message_name == "ClimateControl":
                    ...
                    if signals["ACStatus"] == 1 and signals["FanSpeed"] == 0:
                        print(f"      ├─ ⚠️ Hatalı veri: Klima açık iken fan hızı 0 olamaz")
                    ...
                    if signals["Speed"] > 120:
                        print(f"      ├─ ⚠️ Yüksek hız, lütfen yavaşlayınız. ({signals['Speed']:.1f} km/s)")
    ...
    def update_vehicle_state(self, vehicle_id: str, message_data: Dict[str, Any], geography: Geography):
        ...
        self._check_isolation_forest_anomalies(message_name, signals)
    ...
    def _check_geography_based_anomalies(self, state: VehicleState, message_name: str, signals: Dict[str, float]):
        ...
        if state.geography == Geography.RAINY:
            if message_name == "VehicleData" and signals.get("Speed", 0) > 70:
                print("\n🌧️  YAĞMURLU HAVA ANOMALİSİ")
                print(f"   └─ Yüksek hız: {signals['Speed']:.1f} km/s")
        
        elif state.geography == Geography.MOUNTAINOUS:
            if message_name == "EngineData" and signals.get("EngineTemp", 0) > 95:
                print("\n⛰️  DAĞLIK BÖLGE ANOMALİSİ")
                print(f"   └─ Yüksek motor sıcaklığı: {signals['EngineTemp']:.1f}°C")
            
            if message_name == "VehicleData" and signals.get("Speed", 0) > 70:
                print("\n⛰️  DAĞLIK BÖLGE ANOMALİSİ")
                print(f"   └─ Yüksek hız uyarısı: {signals['Speed']:.1f} km/s")
        
        elif state.geography == Geography.HOT:
            if message_name == "EngineData" and signals.get("EngineTemp", 0) > 100:
                print("\n🌡️  SICAK BÖLGE ANOMALİSİ")
                print(f"   └─ Yüksek motor sıcaklığı: {signals['EngineTemp']:.1f}°C")
            
            if message_name == "ClimateControl":
                if signals.get("CabinTemp", 20) > 30:
                    print("\n🌡️  SICAK BÖLGE ANOMALİSİ")
                    print(f"   └─ Yüksek kabin sıcaklığı: {signals['CabinTemp']:.1f}°C")
        
        elif state.geography == Geography.SNOWY:
            if message_name == "VehicleData" and signals.get("Speed", 0) > 50:
                print("\n❄️  KARLI HAVA ANOMALİSİ")
                print(f"   └─ Yüksek hız: {signals['Speed']:.1f} km/s")
            
            if message_name == "ClimateControl" and signals.get("CabinTemp", 20) < 15:
                print("\n❄️  KARLI HAVA ANOMALİSİ")
                print(f"   └─ Düşük kabin sıcaklığı: {signals['CabinTemp']:.1f}°C")
        
        elif state.geography == Geography.URBAN:
            if message_name == "VehicleData" and signals.get("Speed", 0) > 60:
                print("\n🏙️  ŞEHİR İÇİ ANOMALİSİ")
                print(f"   └─ Hız limiti aşımı: {signals['Speed']:.1f} km/s")
            
            if message_name == "EngineData" and signals.get("EngineSpeed", 0) > 4000:
                print("\n🏙️  ŞEHİR İÇİ ANOMALİSİ")
                print(f"   └─ Yüksek motor devri: {signals['EngineSpeed']:.1f} RPM")

        elif state.geography == Geography.HIGHWAY:
            if message_name == "VehicleData" and signals.get("Speed", 0) < 60:
                print("\n🛣️  OTOYOL ANOMALİSİ")
                print(f"   └─ Düşük hız: {signals['Speed']:.1f} km/s")
    ...
    def _check_temporal_anomalies(self, state: VehicleState, message_name: str, signals: Dict[str, float]):
        ...
        if message_name == "VehicleData":
            ...
            if speed_diff > 20:
                print("\n⚠️  HIZ ANOMALİSİ")
                print(f"   ├─ Önceki hız: {last_signals['Speed']:.1f} km/s")
                print(f"   ├─ Yeni hız: {signals['Speed']:.1f} km/s")
                print(f"   └─ Değişim: {speed_diff:.1f} km/s")
            ...
            if temp_diff > 15:
                print("\n🌡️  MOTOR SICAKLIĞI ANOMALİSİ")
                print(f"   ├─ Önceki sıcaklık: {last_signals['EngineTemp']:.1f}°C")
                print(f"   ├─ Yeni sıcaklık: {signals['EngineTemp']:.1f}°C")
                print(f"   └─ Değişim: {temp_diff:.1f}°C")
            
            # Ani motor devri değişimi
            ...
            # Ani batarya seviyesi düşüşü
            battery_diff = last_signals["BatteryLevel"] - signals["BatteryLevel"]
            if battery_diff > 20:  # %20'dan fazla ani düşüş
                print("\n🔋 BATARYA ANOMALİSİ")
    ...
    def _check_signal_based_anomalies(self, signals: Dict[str, float], message_name: str):
        ...
        if message_name == "EngineData":
            ...
            if speed > 0:  # Araç hareket halindeyse
                ...
                elif speed < 50 and current_gear >= 3:
                    print("\n⚠️  KRİTİK VİTES UYUMSUZLUĞU")
